...
export default {
  ...
  mounted() {
    ...
    this.$axios({
      ...
      this.$store.commit("air/setairInfo", this.airData);
    });
  },
  ...
  computed: {
    allPrice() {
      ...
      //   const { airport_tax_audlet, seat_infos, insurances } = this.airData;
      let price = 0;
      ...
      this.$store.commit("air/setairPrice", price);
      return price;
    }
  },
  ...
  methods: {
    ...
    //  获取保险id
    handleChange(id) {
      const Id = this.insurances.indexOf(id);

      if (Id > -1) {
        this.insurances.splice(Id, 1);
      } else {
